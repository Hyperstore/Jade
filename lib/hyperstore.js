...
var SchemaState = (function () {
    function SchemaState(store, config) {
        this.config = config;
        if (store) {
            this.constraints = {};
            this.id = config.id;
    ...
    SchemaState.Pending = new SchemaState(null, "$pending");
    return SchemaState;
})();
...
var Loader = (function () {
    ...
    Loader.prototype.loadSchemas = function (schemas) {
        var _this = this;
        var meta = {};
        if (!schemas)
            return meta;
    ...
    Loader.prototype._resolveSchema = function (id) {
        ...
        Utils.forEach(configs, function (cfg) {
            if (cfg.id === id) {
...
exports.SchemaLoaderException = SchemaLoaderException;

var SchemaParser = (function () {
    function SchemaParser(_loader) {
        this._loader = _loader;
        this._imports = new HashTable();
    }
    SchemaParser.prototype.parse = function (schema) {
        var _this = this;
        if (!schema)
            return undefined;
        ...
        this.parseConstraints(schema.constraints, function (ct) {
            return _this._state.constraints[ct.name] = ct;
        });
    ...
    SchemaParser.prototype.createRelationship = function (def) {
        ...
        if (rel) {
            ...
            rel = new SchemaRelationship(this._state.schema, name, src.id, end.id, c.embedded || false, c.type, undefined, undefined, def.base);
            this._state.meta[name] = rel;
        }

        this.parseConstraints(def.const, function (c) {
            return rel.addConstraint(c.message, c.condition, c.error, c.type);
        });
        ...
        for (var prop in def.obj) {
            if (!def.obj.hasOwnProperty(prop) || prop[0] === "$" || prop === "end" || prop === "kind" || prop === "extends" || prop === "source" || prop === "constraints")
                continue;
    ...
    SchemaParser.prototype.parseProperty = function (name, definition, entity) {
        ...
        if (typeof (definition) === "string") {
            ...
            if (!t)
                throw new SchemaLoaderException("Invalid type " + definition.type + " for property " + name, this._state.schema);
    ...
    SchemaParser.prototype._resolveType = function (name) {
        if (!name)
            return undefined;
        var fullName;
        var parts = name.split('.');
    ...
    SchemaParser.prototype.parseTypes = function (values) {
        var _this = this;
        if (!values)
            return;
...
var AddEntityEvent = (function (_super) {
    ...
    function AddEntityEvent(domain, id, schemaId, version, correlationId) {
        _super.call(this, EventManager.AddEntityEvent, domain, version, correlationId);
        this.id = id;
...
var RemoveEntityEvent = (function (_super) {
    ...
    function RemoveEntityEvent(domain, id, schemaId, version, correlationId) {
        _super.call(this, EventManager.RemoveEntityEvent, domain, version, correlationId);
        this.id = id;
        this.schemaId = schemaId;
    }
...
var AddRelationshipEvent = (function (_super) {
    ...
    function AddRelationshipEvent(domain, id, schemaId, startId, startSchemaId, endId, endSchemaId, version, correlationId) {
        _super.call(this, EventManager.AddRelationshipEvent, domain, version, correlationId);
        this.id = id;
        this.schemaId = schemaId;
        this.startId = startId;
        this.startSchemaId = startSchemaId;
...
var RemoveRelationshipEvent = (function (_super) {
    ...
    function RemoveRelationshipEvent(domain, id, schemaId, startId, startSchemaId, endId, endSchemaId, version, correlationId) {
        _super.call(this, EventManager.RemoveRelationshipEvent, domain, version, correlationId);
        this.id = id;
        this.schemaId = schemaId;
        this.startId = startId;
        this.startSchemaId = startSchemaId;
...
var ChangePropertyValueEvent = (function (_super) {
    ...
    function ChangePropertyValueEvent(domain, id, schemaId, propertyName, value, oldValue, version, correlationId) {
        _super.call(this, EventManager.ChangePropertyValueEvent, domain, version, correlationId);
        this.id = id;
        this.schemaId = schemaId;
...
var RemovePropertyEvent = (function (_super) {
    ...
    function RemovePropertyEvent(domain, id, schemaId, propertyName, value, version, correlationId) {
        _super.call(this, EventManager.RemovePropertyEvent, domain, version, correlationId);
        this.id = id;
        this.schemaId = schemaId;
        this.propertyName = propertyName;
        this.value = value;
...
exports.DomainEventDispatcher = DomainEventDispatcher;
...
var DomainModel = (function () {
    ...
    DomainModel.prototype.setPropertyValue = function (ownerId, property, value, version) {
        ...
        this._raiseEvent(new ChangePropertyValueEvent(this.name, ownerId, ownerNode.schemaId, property.name, property.serialize(pv.value), property.serialize(pv.oldValue), pv.version));
        return pv;
    };
    ...
    DomainModel.prototype.create = function (schemaElement, id, version) {
        ...
        this._raiseEvent(new AddEntityEvent(this.name, id, schemaElement.id, node.version));
        this._cache[id] = mel;
        return mel;
    };
    ...
    DomainModel.prototype.createRelationship = function (schemaRelationship, start, endId, endSchemaId, id, version) {
        ...
        this._raiseEvent(new AddRelationshipEvent(this.name, id, schemaRelationship.id, src.id, src.schemaElement.id, endId, endSchemaId, node.version));
    ...
    DomainModel.prototype._raiseEvent = function (evt) {
        ...
        this.store.runInSession(function () {
            if (Array.isArray(evt)) {
                Utils.forEach(evt, function (e) {
                    Session.current.addEvent(e);
...
var DomainModelScope = (function (_super) {
    ...
    DomainModelScope.prototype.apply = function (dispatcher) {
        var _this = this;
        this.store.runInSession(function () {
            var d = new DomainEventDispatcher(_this.domain, dispatcher || _this.store.eventBus.defaultEventDispatcher);
...
var Hypergraph = (function () {
    ...
    Hypergraph.prototype.removeNode = function (id, version) {
        ...
        this.traverseNodes(node, function (node) {
            ...
            var evt;
            if (!node.startId) {
                evt = new RemoveEntityEvent(_this.domain.name, node.id, node.schemaId, version);
            } else {
                evt = new RemoveRelationshipEvent(_this.domain.name, node.id, node.schemaId, node.startId, node.startSchemaId, node.endId, node.endSchemaId, version);
            }
            evt.TL = node.id === id;
            events.push(evt);
    ...
    Hypergraph.prototype.removeNodeInternal = function (id, sawNodes, events) {
        ...
        schema.getProperties(true).forEach(function (p) {
            ...
            if (pnode) {
                events.push(new RemovePropertyEvent(self.domain.name, node.id, node.schemaId, p.name, pnode.value, pnode.version));
            }
        });
    };
...
var HypergraphEx = (function (_super) {
    ...
    HypergraphEx.prototype.getNodes = function (kind, schema) {
        var _this = this;
        return new ConcatCursor(new NodesCursor(this, kind, schema), new MapCursor(new NodesCursor(this.domain.getGraph(), kind, schema), function (n) {
            return _this._keys[n.id] !== Hypergraph.DELETED_NODE ? n : undefined;
        }));
    };
    ...
    HypergraphEx.prototype.getEdges = function (id, direction) {
        ...
        if (n !== undefined) {
            ...
            return cursor2 ? new ConcatCursor(cursor1, new MapCursor(cursor2, function (n) {
                return _this._keys[n.id] !== Hypergraph.DELETED_NODE ? n : undefined;
            })) : cursor1;
        }
...
var ConcatCursor = (function (_super) {
    ...
    ConcatCursor.prototype.reset = function () {
        ...
        this._set = {};
        this._idx = 0;
        this._cursors.forEach(function (c) {
            return c.reset();
        });
    };
...
var GraphNode = (function (_super) {
    ...
    function GraphNode(id, schemaId, kind, version, startId, startSchemaId, endId, endSchemaId, value, outgoings, incomings) {
        ...
        this.properties = [];
        this.outgoings = outgoings && outgoings.clone() || new HashTable();
        this.incomings = incomings && incomings.clone() || new HashTable();
    }
...
var Store = (function () {
    ...
    Store.prototype.loadSchemas = function (schemas) {
        if (!schemas)
            return null;
    ...
    Store.prototype.unloadDomain = function (domain, commitChanges) {
        if (typeof commitChanges === "undefined") { commitChanges = false; }
        var scope = domain;
    ...
    Store.prototype.primitiveSchemaDefinition = function () {
        return {
            defineSchema: function (schema) {
                new Primitive(schema, "any"), new Primitive(schema, "string", "{value} must be a string", function (val, old, ctx) {
    ...
    Store.prototype.runInSession = function (action, mode) {
        if (typeof mode === "undefined") { mode = 0 /* Normal */; }
...
var ModelElement = (function () {
    ...
    ModelElement.prototype.getParent = function () {
        var rel = this.getRelationships(undefined, 1 /* Incoming */).firstOrDefault(function (r) {
            return r.embedded;
        });
...
var IndexedDbAdapter = (function (_super) {
        ...
        this.open().then(function (db) {
            ...
            elements.forEach(function (element) {
                switch (element.state) {
                    case 0 /* Added */:
                        var data = { id: element.id, schema: element.schemaId, version: element.version };
                        if (element.startId) {
                        ...
                        if (element.properties) {
                            ...
                            for (var pn in element.properties) {
                                ...
                                if (pv && pv.value) {
                                    ...
                                    var data = { va: ps.serialize(pv.value), ve: pv.version, id: element.id + pn };
                                    ostore.put(data);
                                }
                            }
                        }
    ...
    IndexedDbAdapter.DB_NAME = "_HY$DB";
...
var DomainSerializer = (function () {
    ...
    DomainSerializer.save = function (domain, entities, relationships) {
        if (!domain || !(domain instanceof DomainModel))
