...
var DslParser = (function () {
    function DslParser(schema, def, context) {
        this.schema = schema;
        this.def = def;
...
var DomainModel = (function () {
        ...
        if (start) {
            ...
            if (node) {
                return node.outgoings.map(function (info) {
                    ...
                    return _this.getFromCache(tmpSchema, start.id, start.schemaElement.id, info.endId, info.endSchemaId, info.id);
                });
            }
        } else if (end) {
            ...
            if (node) {
                return node.incomings.map(function (info) {
                    ...
                    return _this.getFromCache(tmpSchema, info.endId, info.endSchemaId, end.id, end.schemaElement.id, info.id);
                });
            }
        } else {
    ...
    DomainModel.prototype.createRelationship = function (schemaRelationship, start, endId, endSchemaId, id, version) {
        ...
        var node = this.graph.addRelationship(id, schemaRelationship.id, start.id, start.schemaElement.id, endId, endSchemaId, version);
...
var ModelElementArray = (function () {
    function ModelElementArray(_collection) {
        ...
        this._sessionCompletedCookie = this._collection.domain.events.on(EventManager.SessionCompleted, function (s) {
            ...
            Utils.forEach(s.events, function (e) {
                ...
                if (e.schemaId === self._collection.schemaRelationship.id && (self._collection.source && e.startId === self._collection.source.id) || (self._collection.end && e.endId === self._collection.end.id)) {
    ...
    ModelElementArray.prototype.reset = function () {
        ...
        var cursor = this._collection.domain.findRelationships(this._collection.schemaRelationship, this._collection.source, this._collection.end);
...
var ModelElementCollection = (function (_super) {
    ...
    ModelElementCollection.prototype.add = function (mel) {
        ...
        var rel = this.source.domain.createRelationship(this.schemaRelationship, source, end.id, end.schemaElement.id);
        if (this._items)
            this._items.add(rel);
    };
...
var ConstraintsManager = (function () {
    ...
    ConstraintsManager.prototype.checkOrValidateElements = function (elements, kind) {
        ...
        Utils.forEach(elements, function (mel) {
            try  {
                ctx.element = mel;
                _this.checkCondition(ctx, mel.schemaElement);
            } catch (e) {
                ctx.log(e, 1 /* Error */);
            }
        });
...
var DomainSerializer = (function () {
    ...
    DomainSerializer.prototype.saveInternal = function (entities, relationships) {
        try  {
            this.serializeEntities(entities || this._domain.find(undefined, 1 /* Entity */));
            this.serializeRelationships(relationships || this._domain.find(undefined, 2 /* Relationship */));
...
var FileDomainSerializer = (function () {
    ...
    FileDomainSerializer.save = function (domain, entities, relationships) {
