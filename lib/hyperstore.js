...
var DslParser = (function () {
    ...
    DslParser.prototype.extends = function (v, o) {
        if (!o)
            return;
        for (var p in o) {
...
var SchemaValueObject = (function (_super) {
    ...
    function SchemaValueObject(schema, id, validate, message, kind) {
...
var Primitive = (function (_super) {
    ...
    function Primitive(schema, id, validate, message, checkConstraint) {
        if (typeof checkConstraint === "undefined") { checkConstraint = true; }
        _super.call(this, schema, id, validate, message, checkConstraint ? 0 /* Check */ : 1 /* Validate */);
...
(function (ResolutionState) {
    ResolutionState[ResolutionState["success"] = 1] = "success";
    ResolutionState[ResolutionState["error"] = 2] = "error";
...
var Promise = (function () {
    ...
    Promise.prototype.bind = function (func, target) {
        return function () {
            func.apply(target, arguments);
        };
    };
    Promise.prototype.isGenericPromise = function (obj) {
        return obj && (typeof obj.then === "function");
    };

    Promise.prototype._resolveSuccess = function (val) {
        this._resolve(1 /* success */, val);
    };

    Promise.prototype._resolveError = function (err) {
        this._resolve(2 /* error */, err);
    };
    ...
    Promise.prototype._invokeCallback = function (frame) {
        var callbackToInvoke = this._resolutionState === 1 /* success */ ? frame.success : frame.error;
        if (typeof callbackToInvoke === "function") {
            setTimeout(this.bind(function () {
                ...
                try  {
                    ...
                    passthroughState = 1 /* success */;
                } catch (ex) {
                    callbackDidNotThrow = false;
                    passthroughValue = ex;
                    passthroughState = 2 /* error */;
                }
    ...
    Session.prototype.close = function () {
        ...
        if (!this.aborted && !this.result.hasErrorsOrWarnings)
            this.store.domains.forEach(function (d) {
                return d.events.__notifySessionCompleted(self);
            });
...
var Store = (function () {
        ...
        if (config.domains) {
            for (var domainName in config.domains) {
                ...
                if (typeof (def.adapters) === "function") {
                    var _p = p;
                    def.adapters().forEach(function (a) {
                        var tmp = _p;
    ...
    Store.prototype.getElement = function (id) {
...
var DomainModel = (function () {
    ...
    DomainModel.prototype.addAdapterAsync = function (adapter) {
        var self = this;
    ...
    DomainModel.prototype.getRelationships = function (schemaElement, start, end) {
        ...
        if (start) {
            ...
            return Utils.select(this._graph.getNodes(2 /* Edge */, schemaElement), function (n) {
                tmpSchema = currentSchema;
                if (schemaElement && schemaElement.id !== tmpSchema.id) {
    ...
    DomainModel.prototype.removeElement = function (id, version) {
        var _this = this;
        var events;
        this.store.runInSession(function () {
    ...
    DomainModel.prototype.getElement = function (id) {
...
var Hypergraph = (function () {
    ...
    Hypergraph.prototype.removeNode = function (id, version) {
        ...
        this.traverseNodes(node, function (node) {
            ...
            for (var k in node.outgoings) {
                ...
                if (!sawNodes[edge.id]) {
                    ...
                    nodes.push(_this.nodes[edge.id]);
                }
            }
            ...
            if (node.startId) {
                ...
                if (schema.embedded) {
                    if (!sawNodes[node.endId]) {
                        ...
                        nodes.push(_this.nodes[node.endId]);
                    }
                }
            }
        ...
        var pevents = [];
        events.forEach(function (e) {
            return _this.removeNodeInternal(_this.nodes[e.id], sawNodes, pevents);
        });
        return pevents.concat(events);
    };

    Hypergraph.prototype.removeNodeInternal = function (node, sawNodes, events) {
        if (!node) {
            return;
        }
...
var ConstraintsManager = (function () {
    ...
    ConstraintsManager.prototype.addPropertyConstraint = function (property, condition, message, asError, kind) {
        ...
        if (!fn && property.schemaProperty) {
            var tmp;
            if (property.schemaProperty.check) {
                tmp = property.schemaProperty.check;
                def.kind = 0 /* Check */;
            } else if (property.schemaProperty.validate) {
                tmp = property.schemaProperty.validate;
                def.kind = 1 /* Validate */;
            }
        ...
        this.addConstraint(property.owner, {
            ...
            messageType: def.error ? 1 /* Error */ : 0 /* Warning */,
            verify: function (self, ctx) {
                ...
                try  {
                    ...
                    ctx.log(e, def.error ? 1 /* Error */ : 0 /* Warning */);
                }
    ...
    ConstraintsManager.prototype.checkElement = function (ctx, schemaElement) {
        var constraints = this._constraints[schemaElement.id];
        if (constraints) {
            for (var key in constraints) {
                var constraint = constraints[key];
                if (constraint.kind === 0 /* Check */) {
    ...
    ConstraintsManager.prototype.validateElement = function (ctx, schemaElement) {
        var constraints = this._constraints[schemaElement.id];
        if (constraints) {
...
var DiagnosticMessage = (function () {
    function DiagnosticMessage(messageType, rawMessage, element, propertyName) {
    ...
    Object.defineProperty(DiagnosticMessage.prototype, "message", {
        get: function () {
            if (!this.element) {
            ...
            var self = this;
            var regex = /{\s*(\S*)\s*}/g;
            return this.rawMessage.replace(regex, function (match, name) {
...
var Adapter = (function () {
    ...
    Adapter.prototype.initAsync = function (domain) {
        var _this = this;
...
var IndexedDbAdapter = (function (_super) {
    ...
    IndexedDbAdapter.prototype.initAsync = function (domain) {
        var self = this;
        var promise = new Promise();

        _super.prototype.initAsync.call(this, domain).then(function (_) {
...
var Iterator = (function () {
    ...
    Iterator.prototype.setStart = function (obj) {
        if (obj.length)
            this._list = obj;
        else
            this._list = [obj];
...
var Query = (function () {
    ...
    Query.prototype.moveNext = function () {
        while (true) {
            switch (this._state) {
                case 0:
    ...
    Query.prototype.current = function () {
